...
const pool = mysql.createPool({
  ...
  database: process.env.DB_NAME || 'klecoffee-banco',
...
async function seedAdminUser() {
  ...
  await pool.query(
    ...
    ['Administrador', 'admin@klecoffee.com', hash]
  );
...
app.get('/api/health', async (_req, res) => {
  try {
    ...
    res.status(200).json({ ok: true, message: 'Servidor e MySQL conectados' });
  } catch (error) {
...
app.get('/', (_req, res) => {
  res.status(200).send('KleCoffee online');
});
...
app.get('/api/dashboard', authRequired, async (_req, res) => {
  try {
    ...
    const [maintenanceAlerts] = await pool.query(`
      SELECT id, brand, model, serial_number, next_maintenance
      FROM machines
      WHERE next_maintenance IS NOT NULL
        AND next_maintenance <= DATE_ADD(CURDATE(), INTERVAL 15 DAY)
    ...
    const [recentPayments] = await pool.query(`
      SELECT id, payment_type, customer_name, payment_date, amount, method
      FROM payments
...
app.post('/api/machines', authRequired, upload.single('photo'), async (req, res) => {
  try {
    ...
    await pool.query(`
      ...
      (photo_url, serial_number, value, model, brand, status, last_maintenance, next_maintenance, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      photoUrl,
      serial_number,
...
app.post('/api/events', authRequired, async (req, res) => {
  try {
    ...
    await pool.query(`
      INSERT INTO events
      (client_id, title, location, start_date, end_date, guests_count, estimated_value, machine_id, supplies_notes, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      client_id,
      title,
      ...
      start_date,
      end_date || null,
      toNumber(guests_count),
      toNumber(estimated_value),
      machine_id || null,
      supplies_notes || null,
      status || 'orcamento'
    ]);
...
app.get('/api/products', authRequired, async (_req, res) => {
  try {
    const [rows] = await pool.query(`
      SELECT *,
      ROUND(((sale_price - purchase_price) / NULLIF(purchase_price, 0)) * 100, 2) AS margin_percent
      FROM products
...
app.post('/api/products', authRequired, upload.single('photo'), async (req, res) => {
  try {
    ...
    await pool.query(`
      ...
      (photo_url, name, sku, category, purchase_price, sale_price, stock_quantity, low_stock_alert)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      photoUrl,
      name,
      sku || null,
      category || null,
      toNumber(purchase_price) || 0,
      toNumber(sale_price) || 0,
      Number(stock_quantity || 0),
      Number(low_stock_alert || 5)
    ]);
...
app.post('/api/payments', authRequired, async (req, res) => {
  ...
  try {
    ...
    const [paymentResult] = await connection.query(`
      ...
      (payment_type, reference_id, customer_name, payment_date, amount, method, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [
      payment_type,
      reference_id || null,
    ...
    if (payment_type === 'venda_produto' && Array.isArray(items) && items.length) {
      for (const item of items) {
        ...
        await connection.query(`
          ...
          (payment_id, product_id, quantity, unit_price, line_total)
          VALUES (?, ?, ?, ?, ?)
        `, [
